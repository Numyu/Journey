...
export default function CarouselCard() {
    ...
    const [sliderDirection, setSliderDirection] = useState({right:0, left: -width});
  ...
  return (
    <div>
        <motion.div ref={carousel} className='carousel'>
            <button className="btnViewMore" onClick={changeXY} >View more</button>
        <motion.div drag={slider} dragConstraints={ sliderDirection } className='inner-carousel' style={wrap}>
          {cardCarousel.slice(0, visible).map(({id, title, classCard, img}) => {
            return(
              <motion.div className='item' >
                 <img src ={process.env.PUBLIC_URL + "/images/CarouselImg/" + img}/>
                 <p className="CardItemTitle">{title}</p>
              </motion.div>
            );
          })}
        </motion.div>
